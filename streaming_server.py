...
from pipeline.latency_governor import (
    FreshnessBudgetMs,
    LatencyGovernor,
    LatencyMode,
    QueueLimits,
...
class Config:
    ...
    LATENCY_PROFILE: str = os.environ.get("LATENCY_PROFILE", "normal")
    ...
    # Latency-first queue budgets and freshness cutoffs
    TOKEN_QUEUE_MAX: int = int(os.environ.get("TOKEN_QUEUE_MAX", "4"))
    FRAME_QUEUE_MAX: int = int(os.environ.get("FRAME_QUEUE_MAX", "3"))
    SEND_QUEUE_MAX:  int = int(os.environ.get("SEND_QUEUE_MAX", "5"))
    TOKEN_FRESHNESS_MS: float = float(os.environ.get("TOKEN_FRESHNESS_MS", "220"))
    FRAME_FRESHNESS_MS: float = float(os.environ.get("FRAME_FRESHNESS_MS", "140"))
    SEND_FRESHNESS_MS:  float = float(os.environ.get("SEND_FRESHNESS_MS", "120"))
    GOV_FAST_PRESSURE: float = float(os.environ.get("GOV_FAST_PRESSURE", "0.60"))
    GOV_EMERGENCY_PRESSURE: float = float(os.environ.get("GOV_EMERGENCY_PRESSURE", "0.85"))
    GOV_RECOVER_PRESSURE: float = float(os.environ.get("GOV_RECOVER_PRESSURE", "0.35"))
    GOV_MODE_HOLD_S: float = float(os.environ.get("GOV_MODE_HOLD_S", "1.0"))
...
async def health():
    ...
    return {
        ...
        "queue_limits": {
            "token": cfg.TOKEN_QUEUE_MAX,
            "frame": cfg.FRAME_QUEUE_MAX,
            "send": cfg.SEND_QUEUE_MAX,
        },
...
async def websocket_endpoint(websocket: WebSocket, session_id: str):
    ...
    # ── Per-session latency governor and runtime profile ──────────────────────
    queue_limits = QueueLimits(
        token_q=max(1, cfg.TOKEN_QUEUE_MAX),
        frame_q=max(1, cfg.FRAME_QUEUE_MAX),
        send_q=max(1, cfg.SEND_QUEUE_MAX),
    )
    governor = LatencyGovernor(
        queue_limits=queue_limits,
        thresholds=LatencyThresholds(
            fast_pressure=cfg.GOV_FAST_PRESSURE,
            emergency_pressure=cfg.GOV_EMERGENCY_PRESSURE,
            recover_pressure=cfg.GOV_RECOVER_PRESSURE,
            mode_hold_s=cfg.GOV_MODE_HOLD_S,
        ),
    )
    active_profile = profile_for(cfg.LATENCY_PROFILE)
    freshness = FreshnessBudgetMs(
        token=cfg.TOKEN_FRESHNESS_MS,
        frame=cfg.FRAME_FRESHNESS_MS,
        send=cfg.SEND_FRESHNESS_MS,
    )
    logger.info(
        "[WS] Latency profile=%s | queues(token=%d frame=%d send=%d)",
        cfg.LATENCY_PROFILE,
        queue_limits.token_q,
        queue_limits.frame_q,
        queue_limits.send_q,
    )
    ...
    ditto = DittoStreamAdapter(
        ...
        jpeg_quality = active_profile.jpeg_quality,
    )
    ditto.setup(
        image_path         = image_path,
        emo                = cfg.DITTO_EMO,
        sampling_timesteps = active_profile.sampling_timesteps,
        overlap_v2         = active_profile.overlap_v2,
    )
    ditto.set_runtime_quality(
        jpeg_quality=active_profile.jpeg_quality,
        max_output_fps=active_profile.max_video_fps,
    )
    ...
    token_queue: asyncio.Queue = asyncio.Queue(maxsize=queue_limits.token_q)
    ...
    frame_queue: asyncio.Queue = asyncio.Queue(maxsize=queue_limits.frame_q)

    # send_queue: VIDEO + TEXT only.
    # ws_sender_task drains this queue over websocket.
    send_queue: asyncio.Queue = asyncio.Queue(maxsize=queue_limits.send_q)
    ...
    token_ingest_ts: dict[int, float] = {}
    _token_ts_order: collections.deque = collections.deque(maxlen=2048)
    ...
    def _record_token_ts(seq: int, ts: float):
        token_ingest_ts[seq] = ts
        _token_ts_order.append(seq)
        # keep map bounded
        while len(_token_ts_order) > 1800:
            old = _token_ts_order.popleft()
            token_ingest_ts.pop(old, None)

    def _apply_mode(mode: LatencyMode):
        nonlocal active_profile
        if mode == LatencyMode.NORMAL:
            profile_name = "normal"
        elif mode == LatencyMode.FAST:
            profile_name = "fast"
        else:
            profile_name = "emergency"
        target = profile_for(profile_name)
        ...
        try:
            while True:
                ...
                if item is None:
                    break
                enq_ts, msg = item
                age_ms = (time.monotonic() - enq_ts) * 1000.0
                if age_ms > freshness.send:
                    profiler.record_drop("send_stale", 1)
                    continue
                try:
    ...
    async def bridge_task():
        ...
        """
        # Always start each websocket session with a clean bridge cache.
        ...
        async def _flush():
            ...
            if not token_buffer:
                return
            mode = governor.update(token_queue.qsize(), frame_queue.qsize(), send_queue.qsize())
            _apply_mode(mode)
            throttle_s = governor.bridge_throttle_s()
            if throttle_s > 0:
            ...
            try:
                ...
                _record_token_ts(batch_seq, time.monotonic())

                profiler.log_bridge(
                    chunk_id     = _cid,
                    batch_size   = batch_size,
                    wait_ms      = active_profile.bridge_flush_timeout_ms if _reason == "TIMEOUT" else 0.0,
                    infer_ms     = t_infer.ms,
                    push_ms      = t_push.ms,
        ...
        try:
            while True:
                chunk_size = active_profile.bridge_chunk
                flush_timeout = active_profile.bridge_flush_timeout_ms / 1000.0
                ...
                token_age_ms = (time.monotonic() - item.created_at) * 1000.0
                if token_age_ms > freshness.token:
                    profiler.record_drop("token_stale", 1)
                    continue
                token_buffer.append(item)
                if first_token_time is None:
                    first_token_time = time.monotonic()
                _record_token_ts(item.seq, item.created_at)
            ...
            # Defensive reset so a future session cannot inherit stale KV-cache
            # if this session exits unexpectedly.
            try:
                _run_bridge_reset(bridge)
            except Exception as exc:
                logger.warning(f"[bridge_task] Bridge reset on shutdown failed: {exc}")
            ditto.close()
        ...
        def _safe_enqueue(item):
            ...
            try:
                if item is None:
                    ...
                    frame_queue.put_nowait((time.monotonic(), item))
    ...
    async def frame_forwarder_task():
        ...
        try:
            while True:
                ...
                enq_ts, frame_payload = item
                frame_age_ms = (time.monotonic() - enq_ts) * 1000.0
                if frame_age_ms > freshness.frame:
                    _drop_count += 1
                    profiler.record_drop("frame_stale", 1)
                    continue
                # frame_payload is (seq, jpeg_bytes) from DittoStreamAdapter.iter_frames()
                frame_seq, jpeg = frame_payload
                ...
                try:
                    send_queue.put_nowait((time.monotonic(), hdr + jpeg))
                    token_ts = token_ingest_ts.get(frame_seq)
                    if token_ts is not None:
                        profiler.log_e2e(
                            "token_to_frame_send",
                            (time.monotonic() - token_ts) * 1000.0,
                        )
                except asyncio.QueueFull:
                    _drop_count += 1
        ...
        try:
            while not error_event.is_set():
                ...
                if error_event.is_set():
                    break
                try:
                    send_queue.put_nowait((time.monotonic(), b"\xfe"))  # 1-byte noop keepalive
    ...
    try:
        async for kind, payload in moshi.handle_connection(receive_fn, token_queue):
            ...
            if kind == "handshake":
                ...
                try:
                    send_queue.put_nowait((time.monotonic(), b"\x03" + payload.encode("utf-8")))
                except asyncio.QueueFull:
        ...
        try:
            await send_queue.put((time.monotonic(), b"\xff" + str(exc).encode()))
        except Exception:
            pass
    except Exception as exc:
...
def _parse_args():
    ...
    p.add_argument("--latency-profile", default=None,
                   choices=["normal", "fast", "emergency"])
    ...
    p.add_argument("--token-q",         type=int, default=None)
    p.add_argument("--frame-q",         type=int, default=None)
    p.add_argument("--send-q",          type=int, default=None)
    p.add_argument("--gov-fast-pressure", type=float, default=None)
    p.add_argument("--gov-emergency-pressure", type=float, default=None)
    p.add_argument("--gov-recover-pressure", type=float, default=None)
    p.add_argument("--gov-hold-s", type=float, default=None)
...
if __name__ == "__main__":
    ...
    if args.latency_profile: cfg.LATENCY_PROFILE = args.latency_profile
    ...
    if args.token_q:         cfg.TOKEN_QUEUE_MAX    = args.token_q
    if args.frame_q:         cfg.FRAME_QUEUE_MAX    = args.frame_q
    if args.send_q:          cfg.SEND_QUEUE_MAX     = args.send_q
    if args.gov_fast_pressure is not None:      cfg.GOV_FAST_PRESSURE = args.gov_fast_pressure
    if args.gov_emergency_pressure is not None: cfg.GOV_EMERGENCY_PRESSURE = args.gov_emergency_pressure
    if args.gov_recover_pressure is not None:   cfg.GOV_RECOVER_PRESSURE = args.gov_recover_pressure
    if args.gov_hold_s is not None:             cfg.GOV_MODE_HOLD_S = args.gov_hold_s
