...
class PipelineProfiler:
    ...
    def log_e2e(self, label: str, elapsed_ms: float):
        """Record an end-to-end latency sample."""
        if not self.enabled:
            return
        self.record("E2E", label, elapsed_ms)
    ...
    def print_summary(self, label: str = ""):
        ...
        with self._print_lock:
            ...
            e2e_total = self._averages.get("E2E", {}).get("token_to_frame_send")
            if e2e_total and e2e_total.count() > 0:
                lines.append(
                    f"  E2E token->frame_send: p50={e2e_total.p50():.1f}ms | "
                    f"p95={e2e_total.p95():.1f}ms | p99={e2e_total.p99():.1f}ms"
                )
            lines += [
                "═" * 80,
                "",
