...
class DittoStreamAdapter:
    ...
    def __init__(
        ...
        self._max_output_fps: float = 25.0
    ...
    def set_runtime_quality(self, jpeg_quality: int, max_output_fps: float):
        jpeg_quality = int(max(30, min(95, jpeg_quality)))
        max_output_fps = float(max(8.0, min(30.0, max_output_fps)))
        if jpeg_quality != self.jpeg_quality:
            self.jpeg_quality = jpeg_quality
            self._jpeg_encode = _build_jpeg_encoder(jpeg_quality)
            logger.warning("[DittoStreamAdapter] jpeg_quality -> %s", jpeg_quality)
        self._max_output_fps = max_output_fps
    ...
    def push_features(self, features: np.ndarray, seq: int = 0):
        ...
        # Backpressure-preserving enqueue:
        # never drop bridge features here. Dropping creates irrecoverable gaps
        # because bridge KV-cache has already advanced on the dropped steps.
        # Blocking briefly is intentional so pressure propagates upstream.
        _t_queue_wait = time.perf_counter()
        try:
            a2m_q.put(features, block=True, timeout=1.0)
        except queue.Full:
            logger.error(
                f"[ADAPTER:PUSH] audio2motion_queue still full after 1s (seq={seq}) — "
                "signaling hard backpressure"
            )
            raise RuntimeError("Ditto audio2motion_queue backpressure timeout")
        _queue_wait_ms = (time.perf_counter() - _t_queue_wait) * 1000.0
        ...
        logger.debug(
            ...
            f"a2m_q_before={_q_size_before} | q_wait={_queue_wait_ms:>6.2f}ms | "
    ...
    def iter_frames(self) -> Iterator[Tuple[int, bytes]]:
        ...
        while True:
            ...
            min_dt = 1.0 / max(1.0, self._max_output_fps)
            if _last_emit_ts and (now - _last_emit_ts) < min_dt:
                continue
            _last_emit_ts = now
